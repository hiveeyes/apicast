...
from apicast.core import Result, Station, State
from apicast.format import Formatter


sample_result = Result(
    station=Station(
        state=State(label='Brandenburg', identifier='bifl_bl04'), label='Potsdam', identifier='bifl_0021',
        slug='brandenburg/potsdam'), station_name='Potsdam',
    data=[
...
def test_format_machinify(formatter: Formatter):
    assert formatter.machinify() == [{'date': '2024-03-30', 'evening': 3, 'morning': 2, 'noon': 4},
...
def test_format_translate(formatter: Formatter):
    formatter.translate()
    assert formatter.data == [['date', 'morning', 'noon', 'evening'], ['Sa 30.03.', 'medium', 'intensive', 'strong'],
                              ['So 31.03.', 'medium', 'strong', 'strong'], ['Mo 01.04.', 'medium', 'strong', 'medium']]
